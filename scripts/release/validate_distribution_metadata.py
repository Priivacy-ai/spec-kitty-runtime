...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--package", default="spec-kitty-runtime")
...
def validate_wheel(metadata: email.message.Message, label: str) -> List[str]:
    issues: List[str] = []
...
def validate_sdist(sdist: Path) -> List[str]:
    ...
    if not any(name.endswith("/LICENSE") or name == "LICENSE" for name in names):
        issues.append(f"{sdist.name}: source distribution must include LICENSE")
    return issues
...
def main() -> int:
    ...
    issues: List[str] = []
    for wheel in wheels:
        issues.extend(validate_wheel(read_wheel_metadata(wheel), wheel.name))
    for sdist in sdists:
